...
try:
    ...
    from algorithm import select_optimal_samples, report_progress
except ImportError as e:
    print(f"Error importing algorithm: {e}", file=sys.stderr)
    print("Please ensure the script is run from the project root or adjust sys.path.", file=sys.stderr)
    sys.exit(1)
...
def run_single_benchmark(run_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        algo_result = select_optimal_samples(**full_params, progress_callback=bench_progress_callback)
        ...
        result = {
            ...
            "execution_time": algo_result.get("execution_time", end_time - start_time), # Use reported time if available
            "solver_time": algo_result.get("solver_wall_time", None), # Get solver time if reported
            "num_combos": len(algo_result.get("combos", [])),
            "status": "success",
            "error": None,
            # "progress": "\n".join(progress_log), # Optional: log full progress
        }
        print(f"  Success! Time: {result['execution_time']:.3f}s, Combos: {result['num_combos']}")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark script for Optimal Samples Selection algorithm.")
    parser.add_argument(
        "-n", "--num-runs", type=int, default=DEFAULT_NUM_RUNS,
        help=f"Number of random parameter sets to benchmark (default: {DEFAULT_NUM_RUNS})"
    )
    parser.add_argument(
        "-o", "--output", type=str, default=DEFAULT_OUTPUT_CSV,
