...
const electron_1 = require("electron"); // Import dialog
...
electron_1.ipcMain.handle('get-db-content', async (_event, filename) => {
    ...
    const filePath = path_1.default.join(dbDir, filename);
    let db = null; // Enable DB variable
    try {
        ...
        db = new better_sqlite3_1.default(filePath, { readonly: true, fileMustExist: true }); // Open in read-only, require file to exist
        console.log(`DB Handler: Querying results from ${filename}...`);
        const rows = db.prepare('SELECT * FROM results ORDER BY id').all(); // Get all rows, order by id for consistency
        ...
        if (db && db.open) {
            try {
                db.close();
                console.log(`DB Handler: Closed database ${filePath} after error.`);
            }
            catch (closeErr) { /* Ignore close error */ }
        }
...
electron_1.ipcMain.handle('export-db-to-excel', async (_event, filename) => {
    ...
    const dbFilePath = path_1.default.join(dbDir, filename);
    let db = null;
    try {
        // 1. Read data from DB (similar to get-db-content)
        await promises_1.default.access(dbFilePath);
        db = new better_sqlite3_1.default(dbFilePath, { readonly: true, fileMustExist: true });
        ...
        const samples = JSON.parse(firstRow.samples_json);
        // Extract all combos
        const combos = rows.map(row => JSON.parse(row.combo_json));
