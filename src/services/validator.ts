import { AlgorithmParams } from '@/shared/types'; // Use path alias defined in tsconfig
 ...
 */
export function validateAlgorithmParams(params: AlgorithmParams): void {
 ...
 */
export function quickValidateParamsForUI(params: Partial<{ m: any, n: any, k: any, j: any, s: any, samples: any }>): boolean {
  try {
    ...
    if (isNaN(numM) || isNaN(numN) || isNaN(numK) || isNaN(numJ) || isNaN(numS)) return false;
    if (numM < 45 || numM > 54) return false;
    if (numN < 7 || numN > 25) return false;
    if (numK < 4 || numK > 7) return false;
    if (numS < 3 || numS > 7) return false;
    if (numJ < numS || numJ > numK) return false;

    if (typeof params.samples !== 'string') return false; // Expect comma-separated string for quick check
    const sampleParts = params.samples.split(',').map(p => p.trim()).filter(p => p !== '');
    if (sampleParts.length !== numN) return false; // Check count first
    const samplesNum = sampleParts.map(Number);
    if (samplesNum.some(isNaN)) return false; // Check if all are numbers
    if (new Set(samplesNum).size !== samplesNum.length) return false; // Check uniqueness
    if (!samplesNum.every(sn => sn >= 1 && sn <= numM)) return false; // Check range
